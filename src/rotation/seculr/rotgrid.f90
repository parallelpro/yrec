...
! lay down the equally spaced coordinate rot_diff%chi, then computes the
...
! the rot_diff%chi coordinate. Companion routine to mixgrid.f90 (composition
...
! R (NEEDED FOR I CALCULATION) - STORED IN YVAL
      ...
      do i = 1,ntab
         ii = zone_begin + i - 1
         rot_diff%xtab(i) = rot_diff%chi(i)
         rot_diff%ytab(i) = log_radius(ii)
      end do
      do i = 1,rot_diff%ntot
         rot_diff%xval(i) = rot_diff%echi(i)
      end do
      call osplin(rot_diff%xval,rot_diff%yval,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
! I/MR^2
      do i = 1,ntab
         ii = zone_begin + i - 1
         rot_diff%ytab(i) = moment_of_inertia(ii)/(shell_mass(ii)*radius(ii)**2)
      end do
! MHP 05/02 STORE I/MR^2 FOR LATER USE IN
! DIFFUSION COEFFICIENTS FOR ANGULAR MOMENTUM
! TRANSPORT
      call osplin(rot_diff%xval,eq_reduced_moment_of_inertia,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
! J/M
      do i = 1,ntab
         ii = zone_begin + i - 1
         rot_diff%ytab(i) = specific_angular_momentum(ii)
      end do
      call osplin(rot_diff%xval,eq_angular_momentum,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
! OMEGA
      do i = 1,ntab
         ii = zone_begin + i - 1
         rot_diff%ytab(i) = omega(ii)
      end do
      call osplin(rot_diff%xval,eq_omega,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
! CONVERT TO TOTAL I AND TOTAL J OF THE ZONES.
! INTERMEDIATE POINTS
      do i = 2, rot_diff%ntot-1
         eq_mass(i) = 0.5d0*(rot_diff%es1(i+1)-rot_diff%es1(i-1))
...
! THE INFO PREVIOUSLY STORED IN EI IS NOW IN EI0
         eq_moment_of_inertia(i) = eq_reduced_moment_of_inertia(i)* &
              eq_mass(i)*exp(ln10*2.0d0*rot_diff%yval(i))
...
!
! CENTER
      emtop = 0.5d0*(rot_diff%es1(2)+rot_diff%es1(1))
...
! SURFACE
      embot = 0.5d0*(rot_diff%es1(rot_diff%ntot)+rot_diff%es1(rot_diff%ntot-1))
      if (zone_end.lt.num_zones) then
         emtop = 0.5d0*(enclosed_mass(zone_end)+enclosed_mass(zone_end+1))
      else
         emtop = exp(ln10*log_total_mass)
      end if
      eq_mass(rot_diff%ntot) = emtop - embot
! MHP 05/02 CHANGED TO REFLECT THE FACT THAT
! THE INFO PREVIOUSLY STORED IN EI IS NOW IN EI0
      eq_moment_of_inertia(rot_diff%ntot) = eq_reduced_moment_of_inertia(rot_diff%ntot)* &
           eq_mass(rot_diff%ntot)*radius(zone_end)**2
      eq_angular_momentum(rot_diff%ntot) = eq_angular_momentum(rot_diff%ntot)*eq_mass(rot_diff%ntot)
      if (zone_end.lt.num_zones) then
         do ii = zone_end+1,num_zones
            ...
            eq_mass(rot_diff%ntot) = eq_mass(rot_diff%ntot)+shell_mass(ii)
            eq_moment_of_inertia(rot_diff%ntot) = eq_moment_of_inertia(rot_diff%ntot)+ &
                 moment_of_inertia(ii)
            eq_angular_momentum(rot_diff%ntot) = eq_angular_momentum(rot_diff%ntot)+ &
                 specific_angular_momentum(ii)*shell_mass(ii)
         end do
         i1 = num_zones
 20      continue
...
! BOTTOM INTERFACE OR ABOVE TOP INTERFACE
      ...
      rot_diff%ytab(1) = am_diffusion_coeff(zone_begin+1)
      do i = 2,ntab
         ii = zone_begin + i - 1
         rot_diff%xtab(i) = 0.5d0*(rot_diff%chi(i)+rot_diff%chi(i-1))
         rot_diff%ytab(i) = am_diffusion_coeff(ii)
      end do
      ntabb = ntab + 1
      rot_diff%xtab(ntabb) = rot_diff%chi(ntab)
      rot_diff%ytab(ntabb) = am_diffusion_coeff(zone_end)
      rot_diff%xval(1) = rot_diff%chi(1)
      do i = 2, rot_diff%ntot
         rot_diff%xval(i) = rot_diff%echi(i)-0.5d0*rot_diff%dchi
      end do
      call osplin(rot_diff%xval,eq_am_diffusion_coeff,rot_diff%xtab,rot_diff%ytab,ntabb,rot_diff%ntot)
! DIFFUSION COEFFICIENT FOR MIXING - ASSUME CONSTANT BELOW
! BOTTOM INTERFACE OR ABOVE TOP INTERFACE
      rot_diff%ytab(1) = mixing_diffusion_coeff(zone_begin+1)
      do i = 2,ntab
         ii = zone_begin + i - 1
         rot_diff%ytab(i) = mixing_diffusion_coeff(ii)
      end do
      rot_diff%ytab(ntabb) = mixing_diffusion_coeff(zone_end)
      call osplin(rot_diff%xval,eq_mixing_diffusion_coeff,rot_diff%xtab,rot_diff%ytab,ntabb,rot_diff%ntot)
! ADD DIFFUSION PLUS ADVECTION TREATMENT IF DESIRED
      if (use_diffusion_advection_transport) then
         ...
         rot_diff%ytab(1) = scale_factor*rot_diff%es_advective_velocity(zone_begin + 1)
         do i = 2,ntab
            ii = zone_begin + i - 1
            rot_diff%ytab(i) = scale_factor*rot_diff%es_advective_velocity(ii)
         end do
         rot_diff%ytab(ntabb) = scale_factor*rot_diff%es_advective_velocity(zone_end)
         call osplin(rot_diff%xval,rot_diff%am_advective_coeff,rot_diff%xtab,rot_diff%ytab,ntabb,rot_diff%ntot)
         rot_diff%ytab(1) = scale_factor*rot_diff%es_diffusive_velocity(zone_begin + 1)
         do i = 2,ntab
            ii = zone_begin + i - 1
            rot_diff%ytab(i) = scale_factor*rot_diff%es_diffusive_velocity(ii)
         end do
         rot_diff%ytab(ntabb) = scale_factor*rot_diff%es_diffusive_velocity(zone_end)
         call osplin(rot_diff%xval,rot_diff%am_diffusive_coeff,rot_diff%xtab,rot_diff%ytab,ntabb,rot_diff%ntot)
...
! FROM VERTICAL TRANSPORT.
         do i = 1, rot_diff%ntot
            rot_diff%am_diffusive_coeff(i) = rot_diff%am_diffusive_coeff(i)+ &
...
! STORED IN YVAL
         ...
         rot_diff%ytab(i) = log_density(ii) + log10(dchi_dr) + 2.0d0*log_radius(ii)
      end do
      call osplin(rot_diff%xval,rot_diff%yval,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
...
!         YTAB(I) = YTAB(I) + 2.0D0*HR(II) + LOG10(EI0(I))
         rot_diff%ytab(i) = rot_diff%ytab(i)  + log10(moment_of_inertia(ii)/shell_mass(ii))
      end do
      call osplin(rot_diff%xval,rot_diff%yval,rot_diff%xtab,rot_diff%ytab,ntab,rot_diff%ntot)
! NOW ADD MULTIPLICATIVE FACTORS TO DIFFUSION COEFFICIENTS
      do i = 1, rot_diff%ntot
         eq_am_diffusion_coeff(i) = eq_am_diffusion_coeff(i)*exp(ln10*rot_diff%yval(i))
      end do
! MHP 05/02
      if (use_diffusion_advection_transport) then
         do i = 1,rot_diff%ntot
            rot_diff%am_advective_coeff(i) = rot_diff%am_advective_coeff(i)*exp(ln10*rot_diff%yval(i))
            rot_diff%am_diffusive_coeff(i) = rot_diff%am_diffusive_coeff(i)*exp(ln10*rot_diff%yval(i))
         end do
      end if
! REDEFINE DR AS DCHI
      grid_spacing = rot_diff%dchi
 9999 continue
      return
