...
!  STORE THE OLD VELOCITY ESTIMATES FOR LATER USE.
      if (iteration.gt.1) then
         do 5 i = zone_min,zone_max
            rot_diff%mu_gradient_velocity_prev(i) = circ_vel%mu_gradient_velocity(i)
    ...
    5    continue
         if (use_diffusion_advection_transport) then
            do i = zone_min,zone_max
               rot_diff%es_advective_velocity_prev(i) = rot_diff%es_advective_velocity(i)
               rot_diff%es_diffusive_velocity_prev(i) = rot_diff%es_diffusive_velocity(i)
            end do
         end if
      end if
      do 10 i = 1,num_zones
...
!  TREATMENT OF FIRST INTERFACE.
      if (zone_min.lt.3) then
         rot_diff%kinematic_viscosity_interface(2)=exp(log(mix_phys%viscm(1))* &
              rot_diff%lagrange_interp_weights(1,2)+log(mix_phys%viscm(2))* &
              rot_diff%lagrange_interp_weights(2,2)+ &
              log(mix_phys%viscm(3))*rot_diff%lagrange_interp_weights(3,2)+log(mix_phys%viscm(4))* &
              rot_diff%lagrange_interp_weights(4,2))
         rot_diff%thermal_diffusivity_interface(2)=exp(log(mix_phys%thdifm(1))* &
              rot_diff%lagrange_interp_weights(1,2)+log(mix_phys%thdifm(2))* &
              rot_diff%lagrange_interp_weights(2,2) &
              +log(mix_phys%thdifm(3))*rot_diff%lagrange_interp_weights(3,2)+log(mix_phys%thdifm(4))* &
              rot_diff%lagrange_interp_weights(4,2))
         i0=3
      else
         i0=zone_min
...
!  TREATMENT OF LAST INTERFACE.
      if (zone_max.eq.num_zones) then
         rot_diff%kinematic_viscosity_interface(num_zones)= &
              exp(log(mix_phys%viscm(num_zones-3))*rot_diff%lagrange_interp_weights(1,num_zones)+ &
              log(mix_phys%viscm(num_zones-2))*rot_diff%lagrange_interp_weights(2,num_zones) &
              +log(mix_phys%viscm(num_zones-1))*rot_diff%lagrange_interp_weights(3,num_zones)+ &
              log(mix_phys%viscm(num_zones))*rot_diff%lagrange_interp_weights(4,num_zones))
         rot_diff%thermal_diffusivity_interface(num_zones)= &
              exp(log(mix_phys%thdifm(num_zones-3))* &
              rot_diff%lagrange_interp_weights(1,num_zones)+log(mix_phys%thdifm(num_zones-2))* &
              rot_diff%lagrange_interp_weights(2,num_zones)+log(mix_phys%thdifm(num_zones-1))* &
              rot_diff%lagrange_interp_weights(3,num_zones)+ &
              log(mix_phys%thdifm(num_zones))*rot_diff%lagrange_interp_weights(4,num_zones))
         i1=num_zones-1
      else
         i1=zone_max
      end if
!  GENERAL CASE.
      do 20 i = i0,i1
         rot_diff%kinematic_viscosity_interface(i)=exp(log(mix_phys%viscm(i-2))* &
              rot_diff%lagrange_interp_weights(1,i)+log(mix_phys%viscm(i-1)) &
              *rot_diff%lagrange_interp_weights(2,i)+log(mix_phys%viscm(i))* &
              rot_diff%lagrange_interp_weights(3,i)+log(mix_phys%viscm(i+1)) &
              *rot_diff%lagrange_interp_weights(4,i))
         rot_diff%thermal_diffusivity_interface(i)=exp(log(mix_phys%thdifm(i-2))* &
              rot_diff%lagrange_interp_weights(1,i)+log(mix_phys%thdifm(i-1)) &
              *rot_diff%lagrange_interp_weights(2,i)+log(mix_phys%thdifm(i))* &
              rot_diff%lagrange_interp_weights(3,i)+log(mix_phys%thdifm(i+1)) &
              *rot_diff%lagrange_interp_weights(4,i))
   20 continue
! USE LINEAR INTERPOLATION FOR OMEGA AND MU.
      do i = 2,num_zones
         rot_diff%mean_molecular_weight_interface(i) = 0.5d0*(mix_phys%amum(i)+mix_phys%amum(i-1))
         rot_diff%omega_interface(i) = 0.5d0*(omega(i)+omega(i-1))
      end do
...
! ORIGINAL ESTIMATE,USING DG/G = W**2 R**3 / GM.
         circ_vel%es_circulation_velocity(i) = rot_diff%omega_interface(i)**2* &
              (rot_diff%es_velocity_coeff1(i)+rot_diff%omega_interface(i)**2*rot_diff%es_velocity_coeff2(i))
! QUADRUPOLE TERM ADDED, DG/G COMPUTED AS PER ZAHN 1992.
         dr = radius(i) - radius(i-1)
         gmid = 0.5d0*(rot_diff%local_gravity(i)+rot_diff%local_gravity(i-1))
         phistd = 2.0d0*cc23*rot_diff%omega_interface(i)**2*rot_diff%interface_radius(i)/gmid
         phis2 = cc13*rot_diff%omega_interface(i)**2*(radius(i)**2/rot_diff%local_gravity(i)- &
                 radius(i-1)**2/rot_diff%local_gravity(i-1))/dr
         phi2 = (rot_diff%quadrupole_moment(i-1)/rot_diff%local_gravity(i-1)- &
              rot_diff%quadrupole_moment(i)/rot_diff%local_gravity(i))/dr
         qw = rot_diff%omega_interface(i)*(omega(i)-omega(i-1))/dr
         rot_diff%circulation_correction_ratio(i) = (phis2+phi2)/phistd
         if (rot_diff%circulation_correction_ratio(i).lt.0.0d0) then
            write(*,303) i,rot_diff%circulation_correction_ratio(i),phistd,phis2,phi2, &
            rot_diff%quadrupole_moment(i),rot_diff%quadrupole_moment(i-1),rot_diff%local_gravity(i), &
            rot_diff%local_gravity(i-1),radius(i), &
            radius(i-1)
 303        format(i5,' RAT ',1pe10.3,' POT SPH,CYL,QUA', &
         ...
         if (.not.use_diffusion_advection_transport) then
            circ_vel%es_circulation_velocity(i) = &
                 abs(rot_diff%circulation_correction_ratio(i)*circ_vel%es_circulation_velocity(i)- &
                 rot_diff%es_shear_coeff(i)*qw)
         else
! MHP 05/02 ADD FACTOR OF 1/5 HERE
!               VESA(I) = RAT(I)*VES(I)
!               VESD(I) = ABS(FES3(I)*WM(I)**2)
! MHP 06/02 ADDED D THETA/DT TERM
            rot_diff%theta_new(i) = rot_diff%es_relaxation_factor(i)*(rot_diff%theta_mean(i)*qw- &
                 rot_diff%theta_prev(i))/timestep
            rot_diff%es_advective_velocity(i) = 0.2d0* &
                 (rot_diff%circulation_correction_ratio(i)*circ_vel%es_circulation_velocity(i)+ &
                 rot_diff%theta_new(i))
            q1 = rot_diff%difad_shear_coeff1(i)*rot_diff%omega_interface(i)**2
            q2 = rot_diff%difad_shear_coeff2(i)*qw
            q0 = rot_diff%es_shear_coeff(i)*rot_diff%omega_interface(i)**2
!               VESD(I) = 0.2D0*ABS(FES3(I)*WM(I)**2)
!               VESD(I) = 0.2D0*ABS(Q0+Q1+Q2)
            rot_diff%es_diffusive_velocity(i) = 0.2d0*(q0+q1+q2)
! SECOND ORDER TERM
            rot_diff%vesd2(i) = 0.2d0*rot_diff%facd2(i)*rot_diff%omega_interface(i)
! THIRD ORDER TERM
            rot_diff%vesd3(i) = 0.2d0*rot_diff%facd3(i)*rot_diff%omega_interface(i)
!               VES(I) = ABS(RAT(I)*VES(I)+THN(I)-FES3(I)*QW)
            circ_vel%es_circulation_velocity(i) = &
                 rot_diff%circulation_correction_ratio(i)*circ_vel%es_circulation_velocity(i)+ &
                 rot_diff%theta_new(i)+ &
                 (rot_diff%es_shear_coeff(i)+rot_diff%difad_shear_coeff1(i))*qw+ &
                 rot_diff%difad_shear_coeff2(i)*((omega(i)-omega(i-1))/dr)**2
            rot_diff%second_deriv_geom_factor_eqgrid(i) = &
                 rot_diff%second_deriv_geom_factor(i)*rot_diff%omega_interface(i)
         end if
   31 continue
!      ENDIF
...
! MHP 06/02 REPLACE WITH THE ZAHN&MAEDER 1998 PRESCRIPTION
       do i = zone_min,zone_max
          ...
          ddel=rot_diff%del_grad_diff_interface(i)+ qmu/qp
          ddtest = max(rot_diff%del_grad_diff_interface(i),1.0d-3)
          ddtest2 = max(ddel,1.0d-3)
          qqq = ddtest/ddtest2
          circ_vel%es_circulation_velocity(i) = circ_vel%es_circulation_velocity(i)*qqq
          if (use_diffusion_advection_transport) then
             rot_diff%es_advective_velocity(i) = rot_diff%es_advective_velocity(i)*qqq
             rot_diff%es_diffusive_velocity(i) = rot_diff%es_diffusive_velocity(i)*qqq
             rot_diff%vesd2(i) = rot_diff%vesd2(i)*qqq
             rot_diff%vesd3(i) = rot_diff%vesd3(i)*qqq
          end if
          q1 = rot_diff%difad_shear_coeff1(i)*rot_diff%omega_interface(i)**2*qqq
          q0 = rot_diff%es_shear_coeff(i)*rot_diff%omega_interface(i)**2*qqq
          dr = radius(i) - radius(i-1)
          v2 = 0.2d0*(q0+q1)*(omega(i)-omega(i-1))/dr/rot_diff%omega_interface(i)
! ADD MU GRADIENTS TO VELOCITY ESTIMATES
          rot_diff%es_relaxation_factor(i) = rot_diff%es_relaxation_factor(i)*qqq
          rot_diff%velocity_coeff0(i) = rot_diff%velocity_coeff0(i)*qqq
...
! MU GRADIENTS.
         if (gsf_inhibition_mode.eq.2 .or. gsf_inhibition_mode.eq.0) then
            qwrmx = 2.0d0*sqrt(rot_diff%kinematic_viscosity_interface(i)/ &
                 rot_diff%thermal_diffusivity_interface(i))*dlnomega_dlnr_max(i)
         ...
         if (gsf_inhibition_mode.eq.0) then
            qwrmx=2.0d0*sqrt(rot_diff%interface_gravity_factor(i)* &
                 abs(mix_phys%amum(i)-mix_phys%amum(i-1)) &
                 /dr/rot_diff%mean_molecular_weight_interface(i))
            if (abs(dlnomega_dlnr(i)).lt.qwrmx) then
               circ_vel%gsf_circulation_velocity(i) = 0.0d0
               goto 40
            else
...
! VELOCITIES ARE LATER MULTIPLIED BY R (THE "LENGTH SCALE").
            circ_vel%gsf_circulation_velocity(i) = rot_diff%gsf_kippenhahn_coeff(i)*fx* &
                 rot_diff%thermal_diffusivity_interface(i)*rot_diff%omega_interface(i)**2/rmid
         else
            circ_vel%gsf_circulation_velocity(i)=0.25d0*rot_diff%gsf_kippenhahn_coeff(i)* &
                 rot_diff%thermal_diffusivity_interface(i)*dlnwdr* &
                 rot_diff%omega_interface(i)**2/rmid
         end if
...
!  VALUE AND COMPUTE (LARGE) DYNAMICAL SHEAR VELOCITY.
         if (abs(dlnomega_dlnr(i)).gt.dlnomega_dlnr_max(i)) then
            ...
            circ_vel%secular_shear_velocity(i)=8.0d0/4.5d1* &
                 rot_diff%thermal_diffusivity_interface(i)* &
                 (qwr/dlnomega_dlnr_max(i))**2/rot_diff%interface_radius(i)
...
! THE VELOCITY ESTIMATE HERE IS FROM ZAHN 1991.
         qwrmx = sqrt(rot_diff%kinematic_viscosity_interface(i)/ &
              rot_diff%thermal_diffusivity_interface(i)*1.25d-1*critical_reynolds)* &
              dlnomega_dlnr_max(i)
         if (abs(dlnomega_dlnr(i)).gt.qwrmx) then
!  UNSTABLE; CHECK FOR MU GRADIENTS.
            if (abs((mix_phys%amum(i)-mix_phys%amum(i-1))/rot_diff%mean_molecular_weight_interface(i)) &
                 .lt.1.0d-10) then
               qwrmx2 = 0.0d0
...
!      FACT = (RHOM/PM)*QTMU*DMU/AMUMI/DP*HGM**2
               qwrmx2 = 2.0d0*sqrt(max(1.0d-20,rot_diff%mu_gradient_richardson_coeff(i)* &
                        abs((mix_phys%amum(i)-mix_phys%amum(i-1))/ &
                        rot_diff%mean_molecular_weight_interface(i))))
...
! REPEAT FOR DIF+AD
      if (use_diffusion_advection_transport) then
         do i = zone_min,zone_max
               rot_diff%es_diffusive_velocity(i)=abs(es_velocity_scale* &
                    rot_diff%es_diffusive_velocity(i))
               rot_diff%es_advective_velocity(i)=es_velocity_scale* &
                    rot_diff%es_advective_velocity(i)
         end do
      end if
!               IF(IMU.NE.2)THEN
!                  FM = 1.0D0+VMU(I)
...
! IT AS AN ENTRY IN THE DIFFUSION EQUATION.
         do i = zone_min,zone_max
            rot_diff%es_diffusive_velocity(i) = rot_diff%es_diffusive_velocity(i)+ &
                 rot_diff%interface_radius(i)*(abs(circ_vel%gsf_circulation_velocity(i))+ &
                 abs(circ_vel%secular_shear_velocity(i)))
            rot_diff%shear_diffusion_coeff(i) = rot_diff%interface_radius(i)* &
                 abs(circ_vel%secular_shear_velocity(i))
            rot_diff%gsf_diffusion_coeff(i) = rot_diff%interface_radius(i)* &
...
! MHP 11/94
         if (use_diffusion_advection_transport) then
            do i = zone_min,zone_max
               rot_diff%es_advective_velocity(i) = 0.5d0*(rot_diff%es_advective_velocity(i) &
                    + rot_diff%es_advective_velocity_prev(i))
               rot_diff%es_diffusive_velocity(i) = 0.5d0*(rot_diff%es_diffusive_velocity(i) &
                    + rot_diff%es_diffusive_velocity_prev(i))
            end do
         end if
      end if
...
! THE H-BURNING SHELL.
      if (no_am_transport_in_core) then
         do i = zone_min,zone_max
            dell = rot_diff%interface_luminosity(i)/total_luminosity
            if (dell.lt.9.9d-1) then
               ...
               circ_vel%secular_shear_velocity(i) = 0.0d0
! MHP 11/94
               rot_diff%es_advective_velocity(i) = 0.0d0
               rot_diff%es_diffusive_velocity(i) = 0.0d0
            else
               goto 81
            end if
         end do
