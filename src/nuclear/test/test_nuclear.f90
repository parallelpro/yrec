...
! test_nuclear
!----------------------------------------------------------------------
! Standalone nuclear-domain test program (2026), companion to
...
! Results print for byte-comparison against expected_test_nuclear.out.
program test_nuclear
      use const_lib
      use luout_lib
      ...
      if (len_trim(yrec_input) == 0) then
         write(*,'(a)') "test_nuclear: FAIL (YREC_INPUT not set)"
         stop 1
      end if
...
! unit numbers, per core/parmin.f90
      short_file_unit = 20
      fermi_unit = 15
      open(short_file_unit, file="test_nuclear.short", status="replace")
...
! everything gated off; only the Fermi table is a hard requirement
      ...
      if (setups_ierr /= 0) then
         write(*,'(a)') "test_nuclear: FAIL (setups error)"
         stop 1
      end if
...
! Itoh neutrino losses (pure; first call initializes its own tables)
      write(*,'(a)') "# test_nuclear: sneut (Itoh et al. 1996)"
      do ipt = 1, npts
