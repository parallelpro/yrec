...
!     opacity_table%kurucz2_ix_t / kurucz2_ix_rho ARE THE WARM-START
...
!     FOR SIX GRID POINTS OF TEMPERATURE
      ...
      do temp_index = temp_index_start, temp_index_end
         density_found = .false.
         row_density_start = opacity_table%kurucz2_density_start_index(temp_index)
         row_density_end = row_density_start + &
              opacity_table%kurucz2_density_count(temp_index) - 1
!        FIND THE 'M3'
         ...
         if (log10_density.lt.opacity_table%kurucz2_log10_rho(temp_index, density_pointer)) then
...
!        NOW, (KNOT,KNOT+1) IS SUB-RANGE OF RHO WHICH CONTAINS D.
         delta_logrho = log10_density - opacity_table%kurucz2_log10_rho(temp_index, opacity_table%kurucz2_ix_rho)
         c1 = opacity_table%kurucz2_spline_coeffs(temp_index, coeff_base_index+1)
         c2 = opacity_table%kurucz2_spline_coeffs(temp_index, coeff_base_index+2)
         c3 = opacity_table%kurucz2_spline_coeffs(temp_index, coeff_base_index+3)
         c4 = opacity_table%kurucz2_spline_coeffs(temp_index, coeff_base_index+4)
