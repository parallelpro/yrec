...
! TEMPERATURE INTERPOLATION FACTORS.
      do row = 1,atm_table_nt
         if (log10_teff.le.kurucz_teff_table(row)) exit
      end do
      if (row > atm_table_nt) then
      row = atm_table_nt
      end if
      row_base = max(1,row-2)
      row_base = min(atm_table_nt-3,row_base)
      do k = 1,4
...
! OR LINEAR INTERPOLATION.
         if (kurucz_teff_table(row).gt.logteff_three_point) then
            if (kurucz_teff_table(row).gt.logteff_two_point) then
! LINEAR INTERPOLATION
               fx = (log10_gravity-kurucz_logg_table(atm_table_ng-1))/ &
                    (kurucz_logg_table(atm_table_ng)-kurucz_logg_table(atm_table_ng-1))
               pressure_at_nodes(node) = &
                    kurucz_log10_pressure_table(row,atm_table_ng-1) + &
                    fx*(kurucz_log10_pressure_table(row,atm_table_ng) - &
                    kurucz_log10_pressure_table(row,atm_table_ng-1))
            else
! 3-POINT LAGRANGIAN INTERPOLATION.
               do k = 1,3
                  gravity_nodes_3pt(k) = kurucz_logg_table(atm_table_ng-3+k)
               end do
               call inter3(gravity_nodes_3pt, gravity_weights, &
                    unused_deriv, log10_gravity)
               pressure_at_nodes(node) = &
                    kurucz_log10_pressure_table(row,atm_table_ng-2)*gravity_weights(1) + &
                    kurucz_log10_pressure_table(row,atm_table_ng-1)*gravity_weights(2) + &
                    kurucz_log10_pressure_table(row,atm_table_ng)*gravity_weights(3)
            endif
            cycle
         endif
         if (log10_gravity.ge.kurucz_logg_table(atm_table_ng-1)) then
! DESIRED LOG G ABOVE SECOND TO TOP TABLE LOG G - USE TOP 4 LOG G VALUES.
            do kk = 1,4
               gravity_nodes(kk) = kurucz_logg_table(atm_table_ng-4+kk)
               pressure_table_vals(kk) = &
                    kurucz_log10_pressure_table(row,atm_table_ng-4+kk)
            end do
