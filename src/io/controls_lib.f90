...
! target (io/read_controls.f90 reads into these), grouped by their former
...
! io/read_controls.f90, now illegal there since these are use-associated
...
! /physics/ itself (lnewnuc/weakscreening in io/read_controls.f90, which
...
! for them in io/read_controls.f90 either -- COMMON left them at whatever
...
! (io/read_controls.f90: if(walpcz.lt.-2.0d0) walpcz=-2.0d0, etc.) --
...
! already canonically spelled in io/read_controls.f90 (no rename needed
! there).
...
! io/read_controls.f90's own local names (sli6 etc) are themselves NAMELIST
...
! /physics/ value. io/read_controls.f90's local name for it (lnulos1) is
...
! two members (dsnudt/dsnudd in io/read_controls.f90, neutrino_dlnq_dlnt/
...
! io/read_controls.f90 keeps its local lcalcenv and copies into the
...
! -- io/read_controls.f90 keeps its local lexcom and copies into this
! canonical name.
      logical :: use_extended_composition = .false.

! former common/spots/: all 3 members are NAMELIST /physics/ values,
! same namelist-can't-rename treatment as above -- io/read_controls.f90
...
! disk_locking_active are NAMELIST /physics/ values (io/read_controls.f90's
...
! NAMELIST /physics/ values (io/read_controls.f90's endage/setdt/end_dcen/
...
! io/read_controls.f90 unconditionally assigns them fixed unit numbers at
...
! /physics/ value (io/read_controls.f90's optol, kept local there and
! copy-assigned). zsi/idt/idd (former common/optab/'s remaining
! members) are not namelist values -- dead in io/read_controls.f90 (dropped
...
! io/read_controls.f90 (lphhd is set directly there, not namelist-read, but
...
! io/read_controls.f90 use-associates them directly rather than keeping
! separate locals.
...
! directly in io/read_controls.f90). print_point_interval (originally
! nprtpt) is also a NAMELIST value but needed a different, more
! readable name established elsewhere, so io/read_controls.f90 keeps a
...
! names -- use-associated directly in io/read_controls.f90. chi_grid_scale
! (originally hpttol) is also a NAMELIST value but needed a different,
! more readable name established elsewhere, so io/read_controls.f90 keeps
...
! established elsewhere, so io/read_controls.f90 keeps a local djok and
! copy-assigns.
...
! each with a different canonical spelling than io/read_controls.f90's
! terse names (grtol/ilambda/niter_gs/ldify), so io/read_controls.f90 keeps
...
! each with a different canonical spelling than io/read_controls.f90's
! terse names (dt_gs/xmin/ymin/lthoulfit), so io/read_controls.f90 keeps
! those local and copy-assigns.
...
! io/read_controls.f90 keeps a local ldifz and copy-assigns.
...
! io/read_controls.f90 keeps a local lnewdif and copy-assigns. diffuse_lithium is a
...
! with different canonical spellings than io/read_controls.f90's terse
...
! no declaration-time default; it was unused in io/read_controls.f90 itself
! and dropped there.

! former common/atmos/: atm_choice (originally kttau) is a NAMELIST
! value with a different canonical spelling, kept local in
! io/read_controls.f90 and copy-assigned. NOTE: atm_choice is clobbered
...
! (originally imonte1/imonte2) are not namelist values -- io/read_controls.f90
...
! canonical spellings, so io/read_controls.f90 keeps tridt/tridl local
...
! values and unused in io/read_controls.f90 -- core/read_starting_model.f90 computes
...
! iresca) are not namelist values -- io/read_controls.f90 computes them at
...
! io/read_controls.f90 and copy-assigned.
...
! different canonical spellings, kept local in io/read_controls.f90 and
! copy-assigned.
...
! in io/read_controls.f90, renamed in place there -- so their DATA defaults
...
! -- genuinely used in io/read_controls.f90, renamed in place there -- no
...
! io/read_controls.f90's own CNO/isotope validation logic, renamed in place
...
! spellings, kept local in io/read_controls.f90 and copy-assigned. fo is a
...
! local in io/read_controls.f90 and copy-assigned. ies/imu are NAMELIST
...
! spellings, kept local in io/read_controls.f90 and copy-assigned.
...
! genuinely used in io/read_controls.f90, renamed in place there.
...
! directly; their DATA defaults moved here from io/read_controls.f90
...
! io/read_controls.f90 -- genuinely used in core/check_star_calibration.f90/
...
! io/read_controls.f90, so still declared here for its other users.
...
! io/read_controls.f90, renamed in place there. Their DATA defaults moved
...
! io/read_controls.f90 since DATA can no longer target a use-associated
! entity. use_diffusion_advection_transport (originally ldifad) is a
! NAMELIST value with a different canonical spelling, kept local in
! io/read_controls.f90 and copy-assigned. vfc, the block's per-zone WORK
...
! io/read_controls.f90, renamed in place there.
...
! NAMELIST values with different canonical spellings, kept local in
! io/read_controls.f90 and copy-assigned. allard_table_unit (originally
...
! in io/read_controls.f90 and copy-assigned (pmm_exponent_a/b/c/d/m and
...
! NAMELIST value with a different canonical spelling, kept local in
! io/read_controls.f90 and copy-assigned (also re-synced after the K97/V13
...
! io/read_controls.f90 since DATA can no longer target use-associated
! entities.
