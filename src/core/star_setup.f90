...
! MHP 8/25 directly pass file names instead of using common blocks
      call setups(job%mixture_weights,job%alex06_table_path,job%allard_table_path,job%atm_table_path,job%fermi_table_path,job%kurucz_table_path,job%kurucz_table2_path, &
           job%laol_table_path,job%laol_table2_path,job%opal95_table_path,job%opal92_table_path,job%zams_a_table_path,job%zams_b_table_path,job%zams_c_table_path,job%centre1_table_path,job%centre2_table_path,job%centre3_table_path, &
           job%centre4_table_path,job%centre5_table_path,job%opal92_table2_path,job%pure_z_table_path,job%scv_h_table_path,job%scv_he_table_path,job%scv_z_table_path,job%alex95_table_paths, ierr)
      if (ierr /= 0) return
...
!     OPEN(UNIT=IDYN,FILE=FDYN,FORM='FORMATTED',STATUS='OLD')
         job%mc_run_start = imbeg
         imend = min(imend,1000)
         job%mc_run_end = imend
