...
!     matching henyey_iterate.f90/read_starting_model.f90/check_solar_calibration.f90/check_star_calibration.f90's own
!     slot names for this exact scalar; write_last_model.f90/massloss.f90/
!     rebuild_envelope.f90/write_store_model.f90/write_legacy_output.f90 instead call the same
!     scalar "log_luminosity_lsun".
!   - HL (linear L/Lsun, per-shell array) is named star%luminosity_lsun,
!     matching henyey_iterate.f90/read_starting_model.f90; locate_shell_boundaries.f90/compute_timestep.f90/mix.f90/
!     rezone.f90/rebuild_envelope.f90/write_last_model.f90/write_legacy_output.f90/write_store_model.f90/
!     massloss.f90/evolve_angular_momentum.f90 instead call the same array
!     "log_luminosity" (or plain "luminosity"/"star%luminosity_lsun" in
!     locate_shell_boundaries.f90's own case) -- a misnomer inherited from those files'
...
!     lirate88/liburn/getnewenv/mixcz); read_starting_model.f90/write_last_model.f90/
!     write_legacy_output.f90/write_store_model.f90 instead call it num_shells,
!     henyey_iterate.f90/rotation_shape_factors.f90/locate_shell_boundaries.f90/compute_timestep.f90 call it num_points,
!     write_monte_carlo_model.f90 keeps it cryptic as "m".
!   - JENV (envelope CZ base zone index) is named
!     star%envelope_cz_bottom_index, matching write_last_model.f90/write_legacy_output.f90/
!     write_store_model.f90/write_monte_carlo_model.f90 (majority); locate_shell_boundaries.f90 calls it
!     envelope_edge, find_convection_zones.f90/mix.f90 call it envelope_cz_edge,
!     rezone.f90 calls it convective_envelope_edge_zone,
!     massloss.f90 calls it envelope_boundary_zone, read_starting_model.f90 calls
!     it envelope_zone_index.
!   - JCORE (core CZ top zone index) is named star%core_cz_top_index,
!     matching write_last_model.f90/write_legacy_output.f90/write_store_model.f90/write_monte_carlo_model.f90
!     (majority); locate_shell_boundaries.f90 calls it core_edge, find_convection_zones.f90/mix.f90
!     call it core_cz_edge, rezone.f90 calls it
!     convective_core_edge_zone, compute_timestep.f90 calls it
!     convective_core_edge_zone as well.
!   - LARGE (T if the correction routine has diverged / the model has
!     failed) is named model_diverged_flag; henyey_iterate.f90's own slot name
!     for this same local is corrections_too_large, read_starting_model.f90's is
!     model_failed_flag.
!   - LNEW (T if a fresh envelope-fit triangle must be generated) is
!     named recompute_envelope_triangle; henyey_iterate.f90's own slot name is
!     start_new_triangle, read_starting_model.f90's is envelope_recomputed_flag,
!     rebuild_envelope.f90's is new_points_added_flag.
!   - LRESET is named reset_triangle, matching henyey_iterate.f90 (used in 4 of
!     its 5 call sites here); rezone.f90's own slot name is
!     point_reset_flag.
!   - DELTSH is a dual-purpose scratch slot: read_starting_model.f90 treats it as
!     delta_time_abs (= |delta_time|, set by starin itself) while
!     compute_timestep.f90 treats it as hydrogen_dt (the H-burning-shell
!     timestep governor, computed fresh by htimer). It is named
!     hydrogen_dt here since that is its dominant role (read back by
!     the next model's compute_timestep.f90 call); starin's transient use of the
!     same storage as |delta_time| is noted here for the record.
!   - ISTORE is named istore_flag, matching read_starting_model.f90; rezone.f90's
!     own slot name for the same scalar is envelope_store_index.
!   - SENVOLD (set by massloss, read by getnewenv) is named
!     target_envelope_mass, matching rebuild_envelope.f90 (how it is actually
...
!     read_starting_model.f90 as species_mix_weights (the VNEW mixture-weight
...
!     pulse_mod_path, matching open_pulse_files.f90 (their only consumer besides
!     read_input.f90, which keeps its own dummy spelling fpatm/fpenv/fpmod
...
!     for the same slot, since read_input.f90 keeps its own dummy names at
!     cryptic lowercase spelling only (per that file's NAMELIST
!     constraint, documented in read_input.f90's own header) -- again,
!     positional COMMON/argument passing means this file's own local
!     names are free to differ from read_input.f90's.
!
...
!     "max_iterations" in check_composition.f90, but this file already uses
!     max_iterations as a local name (matching henyey_iterate.f90's own slot
