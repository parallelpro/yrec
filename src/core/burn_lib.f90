...
subroutine dburn(zone_begin, zone_end, num_zones, shell_mass, &
     composition, timestep)
      ...
      if (zone_begin.eq.zone_end) then
         ...
         helium3_fraction = star%xa_start(4,zone_begin)
         rate_start = star%deuterium_burning_rate_start(zone_begin)
         rate_end = star%deuterium_burning_rate(zone_begin)
      else
         total_shell_mass = 0.0d0
         ...
         do zone_idx = zone_begin, zone_end
            ...
            rate_start_sum = rate_start_sum + &
                 shell_mass(zone_idx)*star%deuterium_burning_rate_start(zone_idx)
            rate_end_sum = rate_end_sum + &
                 shell_mass(zone_idx)*star%deuterium_burning_rate(zone_idx)
            hydrogen_fraction = hydrogen_fraction + &
                 star%xa_start(1,zone_idx)*shell_mass(zone_idx)
            deuterium_fraction = deuterium_fraction + &
                 star%xa_start(12,zone_idx)*shell_mass(zone_idx)
            helium3_fraction = helium3_fraction + &
                 star%xa_start(4,zone_idx)*shell_mass(zone_idx)
         end do
...
!        AVERAGE. 1/2 OF THE TIMESTEP.  star%accreted_mass_fraction IS
...
!        AND NEWLY ACCRETED MATTER
         ...
         hydrogen_fraction_new = hydrogen_fraction + 0.5d0* &
              ...
              deuterium_change_accreted*star%accreted_mass_fraction)/ &
              (total_shell_mass + star%accreted_mass_fraction)
...
! TOTAL MASS ACCRETION.
         accreted_mass_fraction_substep = step_fraction*star%accreted_mass_fraction
         deuterium_change_original = deuterium_fraction_burned - &
              deuterium_fraction
         deuterium_change_accreted = accreted_deuterium_burned - &
              star%ctrl%accreted_composition(i_h2)
...
! NEUTRINO COEFFICIENTS
      ...
      data v1,v2,v3/ &
           6.002d19,2.084d20,1.872d21,9.383d-1,-4.141d-1,5.829d-2,5.5924d0, &
           4.886d10,7.580d10,6.023d10,6.290d-3,7.483d-3,3.061d-4,1.5654d0, &
           2.320d-7,8.449d-8,1.787d-8,2.581d-2,1.734d-2,6.990d-4,0.56457d0/
! ***************************
...
!  WERE NOT CHECKED.
      data charge_product/1.d0,4.d0,4.d0,6.d0,6.d0,7.d0,8.d0,12.d0,16.d0,12.d0,14.d0,12.d0,36.d0/, &
           z53/1.175d0, &
           3.73d0,3.73d0,4.804d0,4.804d0,5.385d0,5.941d0,9.014d0,11.24d0,9.014d0,10.15d0, &
           9.104d0,23.28d0/,z43/0.52d0,1.31d0,1.31d0,1.488d0,1.488d0,1.61d0,1.721d0,2.577d0, &
...
!  IN THE EXPRESSIONS FOR PFMC2 AND EFMKT.
      pfmc2=1.017677d-4*pow(electron_number_density_na, (2.0d0/3.0d0))
      efmkt=5.92986d0*t9_m1*(dsqrt(1.d0+pfmc2)-1.d0)
      if (efmkt.le.1.d-2) then
         ...
         if (degd.ge.1.5d0) then
            ...
            fprf=0.75793d0-0.54621d0*degd-0.30964d0*degd**2+0.12535d0*degd**3+ &
            0.1203d0*degd**4-0.012857d0*degd**5-0.014768d0*degd**6
         end if
      end if
...
! THAT U IS PROPORTIONAL TO THE SQUARE ROOT OF RHO TIMES T^(-3/2).
            ...
            dscreen_dlnrho(i)=0.5d0*uwk
            dscreen_dlnt(i)=-1.5d0*uwk
         else
            uint=0.38d0*xxl8*xtr*z86(i)/(ion_mean_weight_inverse*z58*z28)
            if (uwk.le.2.d0) then
               screening_factor(i)=uint
               dscreen_dlnrho(i)=0.43d0*uint
               dscreen_dlnt(i)=-1.29d0*uint
            else
               ustr=0.624d0*z33*xxl6*(z53(i)+0.316d0*z33*z43(i)+0.737d0* &
               z23(i)/(zbar*xxl6))
               if (ustr.lt.uint.or.uwk.ge.5.d0) then
                  screening_factor(i)=ustr
                  dscreen_dlnrho(i)=0.208d0*z33*(z53(i)+0.316d0*z33*z43(i))*xxl6
                  dscreen_dlnt(i)=-3.d0*dscreen_dlnrho(i)
               else
                  screening_factor(i)=uint
                  dscreen_dlnrho(i)=0.43d0*uint
                  dscreen_dlnt(i)=-1.29d0*uint
...
! RATE DERIVATIVES
         ...
         if (reaction_rate(i).lt.1.d-30) then
            ...
            dlnrate_dlnrho(i)=1.d0+dscreen_dlnrho(i)
            dlnrate_dlnt(i)=dscreen_dlnt(i)-(q6(i)*t9_m13+(2.d0*t9_m23+ &
            q1(i)*t9_m13-q3(i)*t9_p13-2.d0*q4(i)*t9_p23-3.d0*q5(i)*t9)/r1)/3.d0+ &
            2.d0*(q8(i)*t9)**2
         end if
      end do
! ***************************************************************
...
!  INTERMEDIATE SCREENING FORMULAE.
         ...
         if (uwk.le.star%ctrl%weak_screening_threshold) then
            ...
            uint = 0.38d0*xxl8*xtr*z86be7p/(ion_mean_weight_inverse*z58*z28)
            utotbe7p = uint
          end if
...
! C13(ALPHA,N) O16
      ...
      a2 = 3.82d5*exp(-9.373d0*t9_m1)
      a3 = 1.41d6*exp(-11.873d0*t9_m1)
      a4 = 2.00d9*exp(-20.409d0*t9_m1)
      a5 = 2.92d9*exp(-29.283d0*t9_m1)
...
! C12(ALPHA,GAMMA)O16
      ...
      dlnrate_dlnt(r_ac12) = dscreen_dlnt(r_ac12)+ &
           ...
           a4*(27.499d0*t9_m1-1.5d0)+a5*(5.0d0+15.541d0*t9_m1))/s_sum
...
! source (~0.5% of the rate at T9 = 0.15).
      r1 = t9_m23+0.012d0*t9_m13+1.45d0+0.117d0*t9_p13+1.97d0*t9_p23 &
           +0.406d0*t9
...
! REACTIONS (EALPCA).
      ...
      star%alpha_capture_energy = 0.0d0
...
! MU SUB E = SUM OVER I [ Z(I)*X(I)/A(I)].
      ion_mean_weight_inverse = 0.d0
      electron_mean_weight_inverse = 0.d0
      xtr = 0.d0
      zeta_sum = 0.d0
      do i = 1,num_isotopes
         ...
         xtr = xtr+term*pow(atomic_number(i), 1.58d0)
...
! CALCULATE WEAK OR INTERMEDIATE SCREENING FOR HEP NEUTRINOS.
         ...
         if (uwk.le.star%ctrl%weak_screening_threshold) then
            ...
            uint = 0.38d0*xxl8*xtr*z86he3p/(ion_mean_weight_inverse*z58*z28)
            utothe3p = uint
         end if
! END OF CALCULATION OF SCREENING CORRECTION FOR HE3 + P REACTION.
         star%neutrino_flux(i_nu_hep) = star%neutrino_flux(i_nu_hep)*exp(utothe3p)
         star%neutrino_flux(i_nu_hep) = star%neutrino_flux(i_nu_hep)*hydrogen_fraction*he3_fraction/ &
              fourpiau2
...
! KC 2025-05-30 CHANGED 1.E-50 TO 0.0 TO AVOID UNDERFLOW
            if (flux_value.le.0.0d0) then
              star%neutrino_flux(k) = 0.0d0
            end if
         end do
...
!  THEIR D/D LN T AND D/D LN RHO ACCUMULATED IN qetnx/qednx.
         ...
         ez3 = ez**3
         emue = 0.5d0*(1.0d0+hydrogen_fraction)
         ex1 = 0.0d0
         if (t9.ge.0.2d0) then
!C PAIR NEUTRINOS
            el2 = el*el
            polx10=(1.d0+el2*(-13.04d0+el2*(133.5d0+el2*(1534.d0+el2*918.6d0))))
...
! MHP 10/02 fixed column 72 problem
            qedn= ez*(v1(2)+2.d0*ez*v1(3))/polx11 - ez*v1(7) &
                  - 3.0d0*ez3/polx12
            qetn= -qedn+ eli*(v1(4)+eli*(2.d0*v1(5)+3.d0*eli*v1(6)))/polx12
            qetnx= el2*(-26.08d0+el2*(534.d0+el2*(9204.d0+el2*7348.8d0)))/polx10
            qetnx = (qetnx + qetn + eli+eli)*ex1
            qednx = (-1.0d0 +cc13*qedn)*ex1
         end if
         qedn= ez*(v2(2)+2.d0*ez*v2(3))/polx21 - ez*v2(7) - 3.d0*ez3/polx22
         qetn= -qedn+ eli*(v2(4)+eli*(2.d0*v2(5)+3.d0*eli*v2(6)))/polx22
         qetnx = qetnx + (5.0d0 + qetn)*ex2
         qednx = qednx +cc13*qedn*ex2
         qedn= ez*(v3(2)+2.d0*ez*v3(3))/polx31 - ez*v3(7) - 3.d0*ez3/polx32
...
! STORE STARTING REACTION RATES.
            ...
            log_rate_li7 = log(star%rate_li7_start(zone_idx))-0.5d0*substep_frac* &
                   (log(star%rate_li7(zone_idx))-log(star%rate_li7_start(zone_idx)))
            log_rate_be9 = log(star%rate_be9_start(zone_idx))-0.5d0*substep_frac* &
...
! NOT STRICTLY TRUE, BUT NOT A BAD APPROXIMATION EITHER.
         if(accretion_active)then
            write(*,913)li6_cz_start,li6_cz_end,li6_accreted,li7_cz_start, &
                 li7_cz_end,li7_accreted,star%accreted_mass_fraction
 913        format(1p7e12.3)
            li6_cz_end = (li6_cz_end*cz_mass_end+li6_accreted*star%accreted_mass_fraction)/ &
                 (cz_mass_end+star%accreted_mass_fraction)
            li7_cz_end = (li7_cz_end*cz_mass_end+li7_accreted*star%accreted_mass_fraction)/ &
                 (cz_mass_end+star%accreted_mass_fraction)
            be9_cz_end = (be9_cz_end*cz_mass_end+be9_accreted*star%accreted_mass_fraction)/ &
                 (cz_mass_end+star%accreted_mass_fraction)
         endif
...
! STORE ENDING RATE FOR USE AT THE BEGINNING OF THE NEXT STEP.
      ...
      star%log_rate_li7_prev = log_rate_li7_cz_end
      star%log_rate_be9_prev = log_rate_be9_cz_end
...
! USE FRAD*RADIATIVE RATE AND (1-FRAD)*CONVECTIVE RATE.
         ...
         li7_depletion = timestep*exp(radiative_frac*log(star%rate_li7(zone_idx))+ &
...
subroutine lirate88(composition, log_density, log_temperature, num_zones, &
     use_current_model)
      ...
      do zone_idx = 1,num_zones
         if(log_temperature(zone_idx).lt.tlim.and.star%logT_start(zone_idx).lt.tlim)exit
...
! SUM RATES
         ...
         if(use_current_model.eq.1)then
            star%rate_li6(zone_idx) = fli6*rhox
            star%rate_li7(zone_idx) = fli7*rhox
            star%rate_be9(zone_idx) = fsbe9*rhox
         else
            star%rate_li6_start(zone_idx) = fli6*rhox
            star%rate_li7_start(zone_idx) = fli7*rhox
            star%rate_be9_start(zone_idx) = fsbe9*rhox
         endif
      end do
      do tail_idx = zone_idx,num_zones
         star%rate_li6(tail_idx)=0.0d0
         star%rate_li7(tail_idx)=0.0d0
         star%rate_be9(tail_idx)=0.0d0
         star%rate_li6_start(tail_idx)=0.0d0
         star%rate_li7_start(tail_idx)=0.0d0
         star%rate_be9_start(tail_idx)=0.0d0
      end do
      return
