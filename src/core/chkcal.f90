...
! Added code to use pre-determined partial derivatives    LLP  6/28/09
      star%run%dlum_dx = -3.78d0            ! empirical result:  -3.783    RMS error .070
...
!     GET IMPROVED GUESSES FOR ALPHA AND X.
         ...
         mixing_length_array(run_index+1) = &
              mixing_length_array(run_index-2)+star%run%delta_alpha
         ...
         rescale_params(2,run_index+1) = rescale_params(2,run_index-2)+star%run%delta_x
